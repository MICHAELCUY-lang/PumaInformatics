<x-admin-layout>
    <div class="mb-12">
        ...
        <h1 class="text-3xl font-serif text-gray-900 tracking-tight">Edit Member</h1>
    </div>

    <form method="POST" action="{{ route('admin.cabinet-members.update', $cabinetMember) }}" enctype="multipart/form-data" class="max-w-4xl">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-12">
            <!-- Name & Basic Info -->
            <div>
                <input type="text" name="name" placeholder="Full Name" class="w-full text-5xl font-serif text-gray-900 border-0 border-b border-gray-200 focus:border-museum-black focus:ring-0 px-0 py-4 placeholder-gray-300 bg-transparent transition-colors" required value="{{ old('name', $cabinetMember->name) }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 border-b border-gray-100 pb-12">
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="role_title" class="w-full border-gray-200 focus:border-museum-black focus:ring-0 rounded-none p-3 text-sm" placeholder="e.g. Vice President" value="{{ old('role_title', $cabinetMember->role_title) }}" required>
                    ...
                    <div>
                        ...
                        <select name="department_id" class="w-full border-gray-200 focus:border-museum-black focus:ring-0 rounded-none p-3 text-sm">
                            ...
                            @foreach($departments as $dept)
                                <option value="{{ $dept->id }}" {{ old('department_id', $cabinetMember->department_id) == $dept->id ? 'selected' : '' }}>{{ $dept->name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="term_year" class="w-full border-gray-200 focus:border-museum-black focus:ring-0 rounded-none p-3 text-sm" placeholder="e.g. 2026-2027" value="{{ old('term_year', $cabinetMember->term_year) }}" required>
                    ...
                    <div>
                        ...
                        <input type="number" name="role_hierarchy_level" class="w-full border-gray-200 focus:border-museum-black focus:ring-0 rounded-none p-3 text-sm" placeholder="10" value="{{ old('role_hierarchy_level', $cabinetMember->role_hierarchy_level) }}">
                    ...
                    <div>
                        <label class="flex items-center space-x-3 cursor-pointer mt-4">
                            ...
                            <input type="checkbox" name="is_active" value="1" class="rounded border-gray-300 text-museum-black focus:ring-museum-black w-5 h-5" {{ old('is_active', $cabinetMember->is_active) ? 'checked' : '' }}>
            ...
            <div class="border-b border-gray-100 pb-12 flex gap-8">
                @if($cabinetMember->getFirstMediaUrl('portrait'))
                    <div class="w-32">
                        <img src="{{ $cabinetMember->getFirstMediaUrl('portrait', 'thumbnail') }}" class="w-full rounded shadow-sm border border-gray-200">
                    </div>
                @endif
                <div class="flex-1">
                    ...
                    <p class="text-xs text-gray-400 mt-1">Recommended: 3:4 aspect ratio, high resolution. Leave empty to keep existing image.</p>
                </div>
            </div>
            ...
            <div>
                ...
                >{{ old('biography', $cabinetMember->biography) }}</textarea>
