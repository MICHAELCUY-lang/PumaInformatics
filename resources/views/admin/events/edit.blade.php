<x-admin-layout>
    <div class="mb-12">
        ...
        <h1 class="text-3xl font-serif text-gray-900 tracking-tight">Edit Event: {{ $event->title }}</h1>
    </div>

    <form method="POST" action="{{ route('admin.events.update', $event) }}" enctype="multipart/form-data" class="max-w-5xl">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-12">
            <!-- Title & Basic Info -->
            <div>
                <input type="text" name="title" placeholder="Event Title" class="w-full text-5xl font-serif text-gray-900 border-0 border-b border-gray-200 focus:border-museum-black focus:ring-0 px-0 py-4 placeholder-gray-300 bg-transparent transition-colors" required value="{{ old('title', $event->title) }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 border-b border-gray-100 pb-12">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        <label class="flex items-center space-x-3 cursor-pointer mt-4">
                            ...
                            <input type="checkbox" name="is_featured" value="1" class="rounded border-gray-300 text-museum-black focus:ring-museum-black w-5 h-5" {{ old('is_featured', $event->is_featured) ? 'checked' : '' }}>
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="datetime-local" name="start_date" class="w-full border-gray-200 focus:border-museum-black focus:ring-0 rounded-none p-3 text-sm" value="{{ old('start_date', $event->start_date ? $event->start_date->format('Y-m-d\TH:i') : '') }}" required>
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="end_date" class="w-full border-gray-200 focus:border-museum-black focus:ring-0 rounded-none p-3 text-sm" value="{{ old('end_date', $event->end_date ? $event->end_date->format('Y-m-d\TH:i') : '') }}">
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="location_name" class="w-full border-gray-200 focus:border-museum-black focus:ring-0 rounded-none p-3 text-sm" placeholder="e.g. Main Auditorium" value="{{ old('location_name', $event->location_name) }}">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="url" name="external_registration_url" class="w-full border-gray-200 focus:border-museum-black focus:ring-0 rounded-none p-3 text-sm" placeholder="https://..." value="{{ old('external_registration_url', $event->external_registration_url) }}">
                    </div>

                    <div>
                        <label class="flex items-center space-x-3 cursor-pointer mt-4">
                            <input type="hidden" name="internal_rsvp_enabled" value="0">
                            <input type="checkbox" name="internal_rsvp_enabled" value="1" class="rounded border-gray-300 text-museum-black focus:ring-museum-black w-5 h-5" {{ old('internal_rsvp_enabled', $event->internal_rsvp_enabled) ? 'checked' : '' }}>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 border-b border-gray-100 pb-12">
                <div>
                    ...
                    <div x-data="{ photoName: null, photoPreview: '{{ $event->getFirstMediaUrl('hero') ?: '' }}' }" class="col-span-6 sm:col-span-4">
                ...
                <div>
                    ...
                    @if($event->hasMedia('gallery'))
                        <div class="flex gap-2 mb-2 overflow-x-auto">
                            @foreach($event->getMedia('gallery') as $media)
                                <img src="{{ $media->getUrl('thumbnail') }}" class="w-16 h-16 object-cover rounded border border-gray-200">
                            @endforeach
                        </div>
                    @endif
                    ...
                    <p class="text-xs text-gray-400 mt-1">Uploading new files will append to existing gallery.</p>
            ...
            <div>
                ...
                <div x-data="tiptapEditor" x-init="initEditor('{!! addslashes(old('description', $event->description)) !!}')" class="border border-gray-200 bg-white min-h-[400px] flex flex-col">
