<x-admin-layout>
    <div class="flex items-center justify-between mb-12">
        <div>
            ...
            <h1 class="text-3xl font-serif text-gray-900 tracking-tight">Project Categories</h1>
            <p class="mt-2 text-sm text-gray-500 font-sans">Organize projects into logical groupings.</p>
        </div>
        <div>
                    ...
                    <tr class="group hover:bg-museum-light/30 transition-colors">
                        ...
                        <td class="py-5 text-right">
                            <div class="flex items-center justify-end space-x-4 opacity-0 group-hover:opacity-100 transition-opacity">
                                <a href="{{ route('admin.project-categories.edit', $category) }}" class="text-sm text-gray-500 hover:text-gray-900">Edit</a>
                                <form action="{{ route('admin.project-categories.destroy', $category) }}" method="POST" class="inline-block" onsubmit="return confirm('Delete this category?');">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="5" class="py-12 text-center">
                            <p class="text-gray-500 font-serif italic">No project categories created yet.</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
    ...
    <x-modal name="create-category" maxWidth="md">
        <form method="POST" action="{{ route('admin.project-categories.store') }}" class="p-8">
            @csrf
            ...
            <div class="mb-5">
                ...
                <input type="text" name="name" class="w-full border-gray-200 focus:border-gray-900 focus:ring-0 rounded-none p-3 text-sm" placeholder="e.g. Web Applications" required>
            </div>
    ...
    <x-modal name="edit-category" maxWidth="md" :show="true">
        <form method="POST" action="{{ route('admin.project-categories.update', $editing) }}" class="p-8">
            @csrf
            @method('PUT')
            ...
            <div class="flex justify-end space-x-4">
                <a href="{{ route('admin.project-categories.index') }}" class="text-sm text-gray-500 hover:text-gray-900">Cancel</a>
