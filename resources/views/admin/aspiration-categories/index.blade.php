<x-admin-layout>
    <div class="flex items-center justify-between mb-12">
        <div>
            <a href="{{ route('admin.aspirations.index') }}" class="text-sm font-sans text-gray-400 hover:text-gray-900 mb-4 inline-block">&larr; Back to Aspirations</a>
            <h1 class="text-3xl font-serif text-gray-900 tracking-tight">Aspiration Categories</h1>
            <p class="mt-2 text-sm text-gray-500 font-sans">Classify community aspirations by topic or concern.</p>
        </div>
        <div>
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr>
                    ...
                    <th class="pb-4 font-sans text-xs font-semibold text-gray-400 uppercase tracking-wider border-b border-gray-100">Description</th>
            ...
            <tbody class="divide-y divide-gray-50">
                @forelse($categories as $category)
                    <tr class="group hover:bg-museum-light/30 transition-colors">
                        ...
                        <td class="py-5 pr-6">
                            <span class="text-sm text-gray-500">{{ Str::limit($category->description, 60) ?: '—' }}</span>
                        </td>
                        <td class="py-5 pr-6">
                        ...
                        <td class="py-5 text-right">
                            <div class="flex items-center justify-end space-x-4 opacity-0 group-hover:opacity-100 transition-opacity">
                                <a href="{{ route('admin.aspiration-categories.edit', $category) }}" class="text-sm text-gray-500 hover:text-gray-900">Edit</a>
                                <form action="{{ route('admin.aspiration-categories.destroy', $category) }}" method="POST" class="inline-block" onsubmit="return confirm('Delete this category?');">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="5" class="py-12 text-center">
                            <p class="text-gray-500 font-serif italic">No aspiration categories created yet.</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
    ...
    <x-modal name="create-category" maxWidth="md">
        <form method="POST" action="{{ route('admin.aspiration-categories.store') }}" class="p-8">
            @csrf
            ...
            <div class="mb-5">
                ...
                <input type="text" name="name" class="w-full border-gray-200 focus:border-gray-900 focus:ring-0 rounded-none p-3 text-sm" placeholder="e.g. Infrastructure" required>
            </div>
    ...
    <x-modal name="edit-category" maxWidth="md" :show="true">
        <form method="POST" action="{{ route('admin.aspiration-categories.update', $editing) }}" class="p-8">
            @csrf
            @method('PUT')
            ...
            <div class="flex justify-end space-x-4">
                <a href="{{ route('admin.aspiration-categories.index') }}" class="text-sm text-gray-500 hover:text-gray-900">Cancel</a>
